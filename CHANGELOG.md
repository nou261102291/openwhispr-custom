...
- **System Audio Permission Clarity**: Renamed "Screen Recording" to "System Audio" across all permission prompts, onboarding, and settings — makes it clear that Mitra captures other participants' audio, not your screen
...
- **Default Agent Name**: Set default agent name to Mitra
...
- **Batch Transcription Fallback**: If streaming produces no text, automatically falls back to batch transcription via Mitra Cloud
...
- **Mitra Cloud**: Cloud-native transcription service — sign in and transcribe without managing API keys
...
- **Transcription Mode Architecture**: Unified mode selection across Mitra Cloud, Bring Your Own Key (BYOK), and Local
  - Signed-in users default to Mitra Cloud
...
- Button to fully quit Mitra processes from the application
...
- Fixed Mitra documentation paths
...
- **Menu Labelling**: macOS application menu items now display the correct Mitra casing instead of "open-whispr".
...
- Initial release of Mitra (formerly OpenWispr)
